...
public class PublisherTopic extends Publisher{
	...
	public PublisherTopic(RabbitWorker rabbitWorker, int publisherId) {
	...
	protected void declareQueues() throws IOException {
		...
		for(String queue: this.rabbitWorker.getQueueManager().getTopicQueues().keySet()) {
			this.channel.queueDeclare(queue, durable, exclusive, autoDelete, null);
			this.channel.queueBind(queue, this.exchange, this.rabbitWorker.getQueueManager().getTopicQueues().get(queue));
		}
	}
	...
	protected String getRandomRountingKey() {
		...
		for(String queue: this.rabbitWorker.getQueueManager().getTopicQueues().keySet()) {
			routingKeys.add(this.rabbitWorker.getQueueManager().getTopicQueues().get(queue));
		}
