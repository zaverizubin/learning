...
public class PublisherFanout extends Publisher{
	...
	public PublisherFanout(RabbitApp rabbitApp, int publisherId) {
	...
	protected void declareQueues() throws IOException {
		...
		for(String queue: this.rabbitApp.getQueueManager().getFanoutQueues().keySet()) {
			this.channel.queueDeclare(queue, durable, exclusive, autoDelete, null);
			this.channel.queueBind(queue, this.exchange, this.rabbitApp.getQueueManager().getFanoutQueues().get(queue));
		}
	}
	...
	protected String getRandomRountingKey() {
		...
		for(String queue: this.rabbitApp.getQueueManager().getFanoutQueues().keySet()) {
			routingKeys.add(this.rabbitApp.getQueueManager().getFanoutQueues().get(queue));
		}
