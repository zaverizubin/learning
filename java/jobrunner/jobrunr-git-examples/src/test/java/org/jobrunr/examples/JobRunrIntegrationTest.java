...
@Import(JobRunrTestConfig.class)
public class JobRunrIntegrationTest {
    ...
    public void givenEndpoint_whenJobEnqueued_thenJobIsProcessedWithin30Seconds() {
        ...
        await()
                ...
                .until(() -> this.storageProvider.getJobById(enqueuedJobId).hasState(SUCCEEDED));
    }
    ...
    public void givenEndpoint_whenJobScheduled_thenJobIsScheduled() {
        ...
        await()
                .atMost(30, TimeUnit.SECONDS)
                .until(() -> this.storageProvider.getJobById(scheduledJobId).hasState(SCHEDULED));
    }

    private String enqueueJobViaRest(String input) {
        return this.restTemplate.getForObject("/enqueue-example-job?name=" + input, String.class);
    }

    private String scheduleJobViaRest(String input, Duration duration) {
        return this.restTemplate.getForObject("/schedule-example-job?name=" + input + "&when=" + duration.toString(), String.class);
    }
}
