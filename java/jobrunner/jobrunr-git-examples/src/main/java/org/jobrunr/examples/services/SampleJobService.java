...
public class SampleJobService {
    ...
    public void recurringJob() throws InterruptedException {
        this.logger.info("The recurring job has begun.");
        try {
            Thread.sleep(5000);
        } catch (InterruptedException e) {
            this.logger.error("Error while executing recurring job", e);
            throw e;
        } finally {
            this.logger.info("Recurring job has finished...");
        }
    }

    @Job(name = "The sample job with variable %0", retries = 2)
    public void executeSampleJob(String input) throws InterruptedException {
        this.logger.info("The sample job has begun. The variable you passed is {}", input);
        try {
            Thread.sleep(1500);
        } catch (InterruptedException e) {
    ...
    public void sampleJobWithRecordInput(SampleJobInput sampleJobInput) throws InterruptedException {
        this.logger.info("The sample job has begun. The variable you passed is {}", sampleJobInput.id());
        try {
            Thread.sleep(1500);
        } catch (InterruptedException e) {
            this.logger.error("Error while executing sample job", e);
