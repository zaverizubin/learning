...
public class JobController {
    ...
    public String enqueueExampleJob(@RequestParam(value = "name", defaultValue = "World") String name) {
        final JobId enqueuedJobId = this.jobScheduler.enqueue(() -> this.sampleService.executeSampleJob("Hello " + name));
    ...
    public String enqueueExampleJobWithRecord(@RequestParam(value = "name", defaultValue = "World") String name) {
        ...
        final JobId enqueuedJobId = this.jobScheduler.enqueue(() -> this.sampleService.sampleJobWithRecordInput(sampleJobInput));
        return "Job Enqueued: " + enqueuedJobId.toString();
    }

    @GetMapping("/delete-job")
    public String deleteExampleJob(@RequestParam(value = "id") String jobId) {
        this.jobScheduler.delete(UUID.fromString(jobId));
    ...
    public String scheduleExampleJob(
        ...
        final JobId scheduledJobId = this.jobScheduler.schedule(now().plus(Duration.parse(when)), () -> this.sampleService.executeSampleJob("Hello " + name));
